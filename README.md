...
- `LeetCode: Start or Join Pairing Session` elects the first participant through a private GitHub Issue, opens that participant's Codespace as host, and automatically joins later participants through Live Share's public extension API. Each GitHub user reuses one visible machine-managed election comment, and older duplicates are cleaned up automatically. Short host leases recover after crashes without storing a LeetCode cookie or GitHub token in the Issue.
- Pairing heartbeats use GitHub's HTTPS API inside the extension process. The extension reuses VS Code's GitHub session, with a one-time encrypted GitHub CLI credential fallback, so Windows does not open a transient terminal for every poll.

Install the pinned VSIX from the [v0.22.15 release](https://github.com/ChaosLights/vscode-leetcode/releases/tag/v0.22.15) in a local VS Code window, not in the Codespace extension host. The `ChaosLights/lc` repository includes a Windows launcher that bootstraps GitHub CLI and browser authentication, then pins this extension, GitHub Codespaces, and Live Share before starting or joining the shared session. Pairing writes UTC JSONL audit events under `%LOCALAPPDATA%\LeetCodePairing\logs`; the companion collector packages and redacts both participants' recent launcher, extension-host, Codespaces, Live Share, and LeetCode logs.
