...
- 从 Explorer 选择 `Show Problem` 时，Codespace 主持人使用 `workspace.fs.createDirectory/writeFile` 和原始 `vscode-remote:` URI 写入云端工作区。Live Share 来宾会把“相对路径 + 题目模板”通过固定版本的 Live Share RPC 发给主持人；主持人端验证路径、大小、读写状态和目标是否已存在后，默认直接通过主持人的工作区 URI 落盘。请求不能覆盖已有文件，也不会使用点击者本地路径。把 `leetcode.liveShare.autoApproveProblemFiles` 设为 `false` 可以恢复逐文件确认。
...
1. 主机和所有来宾都从 [v0.19.1 Release](https://github.com/ChaosLights/vscode-leetcode/releases/tag/v0.19.1) 下载并在各自桌面版 VS Code 本地安装同一个 `vscode-leetcode-live-share-0.19.1.vsix`，不要把它安装到 Codespace 容器。
...
- Live Share RPC 只有在两端都安装本 fork 且主持人正在提供服务时可用。只读会话会拒绝请求；读写请求仍必须通过路径、大小和已存在文件校验。若需要更严格的会话，可以关闭 `leetcode.liveShare.autoApproveProblemFiles`。
