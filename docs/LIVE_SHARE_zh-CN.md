...
1. 主机和所有来宾都使用桌面版 VS Code 1.100.0+ 和 Live Share 1.1.122+，从 [v0.20.0 Release](https://github.com/ChaosLights/vscode-leetcode/releases/tag/v0.20.0) 下载并在各自桌面版 VS Code 本地安装同一个 `vscode-leetcode-live-share-0.20.0.vsix`，不要把它安装到 Codespace 容器。
2. 两端执行 `Developer: Reload Window`。
3. 两端分别执行 `LeetCode: Sign In`，登录各自账号，并选择各自需要的 LeetCode endpoint。
4. 主机开始可读写的 Live Share 会话，来宾打开主机分享的 LeetCode 题目文件。编辑器标题栏的火箭按钮和右键 `LeetCode` 菜单是本地账号操作入口。来宾从 Explorer 创建题目时，由 Live Share 官方文件提供器按来宾权限写入 Codespace，不要求主持人逐次确认。
5. 可通过 `Developer: Show Running Extensions` 确认 LeetCode 运行在每一端的 Local/UI extension host。
6. 遇到问题时执行 `LeetCode: Diagnose Pairing`。输出会列出扩展/VS Code/Live Share/Node 版本、运行位置、Workspace Trust 和各工作区 scheme 的可写性，但不会收集账号名、凭据或完整路径。
...
- 每位参与者需要桌面版 VS Code 1.100.0+、Live Share 1.1.122+、此定制扩展，以及本机 `PATH` 中可执行的外部 Node.js 20+。
...
- v0.20.0 已用 VS Code Extension Host 中的 `vsls:` 模拟提供器覆盖新建、延迟传播、已有文件保护、符号链接拒绝和只读拒绝，并针对本机 Live Share 1.1.122 的当前提供器实现检查。若 Live Share 版本过旧，请先升级而不是安装主持人代理服务。
