...
- 从 Explorer 选择 `Show Problem` 时，Codespace 主持人使用 `workspace.fs.createDirectory/writeFile` 和原始 `vscode-remote:` URI 写入云端工作区。Live Share 来宾不能直接假定拥有创建文件的权限，因此会把“相对路径 + 题目模板”通过固定版本的 Live Share RPC 发给主持人；主持人端验证路径、大小、只读状态和目标是否已存在，弹出确认后再通过主持人的工作区 URI 落盘。请求不能覆盖已有文件，也不会使用点击者本地路径。
...
1. 主机和所有来宾都从 [v0.19.0 Release](https://github.com/ChaosLights/vscode-leetcode/releases/tag/v0.19.0) 下载并在各自桌面版 VS Code 本地安装同一个 `vscode-leetcode-live-share-0.19.0.vsix`，不要把它安装到 Codespace 容器。
2. 两端执行 `Developer: Reload Window`。
3. 两端分别执行 `LeetCode: Sign In`，登录各自账号，并选择各自需要的 LeetCode endpoint。
4. 主机开始 Live Share，会话来宾打开主机分享的 LeetCode 题目文件。编辑器标题栏的火箭按钮和右键 `LeetCode` 菜单是本地账号操作入口。来宾首次从 Explorer 创建某题时，主持人会看到精确目标路径的模态确认框；确认后新文件才会进入 Codespace。
...
| Show Problem（来宾发起） | 确认后打开共享文件 | 主持人先看到目标路径确认框，再实时看到新文件 | 来宾本地账号拉取题面，主持人代理写入 Codespace |

提交后分别刷新两端 LeetCode Explorer，题目状态应只反映各自账号。来宾取消 Test、CLI 报错或 WSL 路径转换失败后，本机临时目录中不应残留 `vscode-leetcode-*` 目录。

## 运行条件

- 每位参与者都需要桌面版 VS Code、此定制扩展和本机 Node.js。
...
- Live Share RPC 只有在两端都安装本 fork 且主持人正在提供服务时可用。只读会话会拒绝请求；读写会话仍要求主持人逐次确认，避免文件生成命令绕过 Live Share 权限边界。
