...
1. 主机和所有来宾都使用桌面版 VS Code 1.100.0+ 和已验证的 Live Share 1.1.122，从 [v0.21.4 Release](https://github.com/ChaosLights/vscode-leetcode/releases/tag/v0.21.4) 下载并在各自桌面版 VS Code 本地安装同一个 `vscode-leetcode-live-share-0.21.4.vsix`，不要把它安装到 Codespace 容器。
...
- v0.21.4 已用 VS Code 1.119 Extension Host 验证本地 CodeLens、`vscode-remote:`/`vsls:` 本地操作、footer 下一行第 0 列的紧凑排版、命令先于 provider 注册、两种远程 scheme 都不再生成可转发的 LeetCode CodeLens、单套四操作、来宾 URI、重复点击和刷新，并继续覆盖新建、删除后重建、Live Share 正负缓存延迟、已有文件保护、符号链接拒绝和只读拒绝。Live Share 已进入维护模式；若版本不同，升级后应重新做双机“无 `no commands`、不重复且各自账号”验收。
