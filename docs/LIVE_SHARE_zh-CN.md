...
1. 主机和所有来宾都使用桌面版 VS Code 1.100.0+ 和已验证的 Live Share 1.1.122，从 [v0.21.1 Release](https://github.com/ChaosLights/vscode-leetcode/releases/tag/v0.21.1) 下载并在各自桌面版 VS Code 本地安装同一个 `vscode-leetcode-live-share-0.21.1.vsix`，不要把它安装到 Codespace 容器。
...
6. 遇到问题时执行 `LeetCode: Diagnose Pairing`。输出会列出扩展/VS Code/Live Share/Node 版本、运行位置、Workspace Trust、各工作区 scheme 的可写性和 CodeLens 可见性，但不会收集账号名、凭据或完整路径。
...
- v0.21.1 已用 VS Code Extension Host 验证主机 CodeLens、来宾 `vsls:` 不注册本地 provider、模拟转发后只有一套、四个操作使用来宾 URI、重复点击和光标恢复，并继续覆盖新建、删除后重建、Live Share 正负缓存延迟、已有文件保护、符号链接拒绝和只读拒绝；实现同时针对本机 Live Share 1.1.122 的命令白名单、参数转换和 30 秒文件缓存检查。Live Share 已进入维护模式；若版本不同，升级后应重新做双机“不重复且各自账号”验收。
