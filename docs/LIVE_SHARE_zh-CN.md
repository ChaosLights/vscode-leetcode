...
2. 来宾看到的是 Live Share 从主机转发的 CodeLens。若来宾本地扩展也注册普通 `vsls:` CodeLens，Live Share 1.1.122 的独占 provider 会屏蔽它；若强行同时注册则会重复。Codespace/UI 扩展宿主之间转发带参数命令时还可能只保留 range、丢失 command，VS Code 因而显示 `no commands`。
...
- 普通本地 `file:`/`untitled:` 文件继续使用原生 CodeLens。Codespaces 主机的 `vscode-remote:` 文档和来宾的 `vsls:` 文档改用本地 clickable inline actions；主机不再为 Codespace 文档生成可被 Live Share 转发的 LeetCode CodeLens，因此来宾不会再收到 `no commands` 占位行。
- Inline actions 直接携带当前窗口本地注册的 Submit/Test/Solution/Description/Star 命令和当前文档 URI，不进入 Live Share 的远程命令通道。主机使用 `vscode-remote:` URI，来宾使用 `vsls:` URI；两端各生成一套且互不转发，所以既不重复，也不会跑到主持人账号。标题栏火箭和右键 `LeetCode` 菜单继续作为兜底。
- Codespaces/Live Share 的可见编辑器在打开、切换和配置变化后做有限的 inline-hint 刷新，处理断线重连后的旧编辑器模型；刷新的是同一个本地 provider，不会产生第二套入口。
...
1. 主机和所有来宾都使用桌面版 VS Code 1.100.0+ 和已验证的 Live Share 1.1.122，从 [v0.21.3 Release](https://github.com/ChaosLights/vscode-leetcode/releases/tag/v0.21.3) 下载并在各自桌面版 VS Code 本地安装同一个 `vscode-leetcode-live-share-0.21.3.vsix`，不要把它安装到 Codespace 容器。
2. 两端执行 `Developer: Reload Window`。
3. 两端分别执行 `LeetCode: Sign In`，登录各自账号，并选择各自需要的 LeetCode endpoint。
4. 主机开始可读写的 Live Share 会话，来宾打开主机分享的 LeetCode 题目文件。`@lc code=end` 旁的 `Submit · Test · Solution · Description` 是本地账号 inline-action 入口；两端应各自只看到一套，并且不应出现 `no commands`。若没有显示，请把 `editor.inlayHints.enabled` 设为 `"on"`，也可使用标题栏火箭或右键 `LeetCode` 菜单。来宾从 Explorer 创建题目时，由 Live Share 官方文件提供器按来宾权限写入 Codespace，不要求主持人逐次确认。
5. 可通过 `Developer: Show Running Extensions` 确认 LeetCode 运行在每一端的 Local/UI extension host。
6. 遇到问题时执行 `LeetCode: Diagnose Pairing`。输出会列出扩展/VS Code/Live Share/Node 版本、运行位置、Workspace Trust、各工作区 scheme 的可写性、当前语言作用域下的 CodeLens/inlay-hint 配置来源，以及实际 provider 返回的按钮数量，但不会收集账号名、凭据或完整路径。
...
来宾不需要把 `liveshare.languages.allowGuestCommandControl` 设置为 `true`。Remote/Live Share inline actions 完全由点击窗口的本地扩展生成和执行，不进入 Live Share 远程命令通道。该设置继续保持默认 `false`，防止其他扩展把主机命令暴露给来宾。
...
| --- | --- | --- | --- |
| 打开题解 | `@lc code=end` 旁只有一套 inline actions | 来宾本地也只有一套，无 `no commands`、不重复 | 尚不调用账号 |
...
- v0.21.3 已用 VS Code 1.119 Extension Host 验证本地 CodeLens、`vscode-remote:`/`vsls:` 本地 inline actions、两种远程 scheme 都不再生成可转发的 LeetCode CodeLens、单套四操作、来宾 URI、重复点击和刷新，并继续覆盖新建、删除后重建、Live Share 正负缓存延迟、已有文件保护、符号链接拒绝和只读拒绝。Live Share 已进入维护模式；若版本不同，升级后应重新做双机“无 `no commands`、不重复且各自账号”验收。
