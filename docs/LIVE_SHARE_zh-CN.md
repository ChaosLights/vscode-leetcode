...
- 本地 CLI 默认由桌面 VS Code 自带的 Node.js 运行时执行，不要求参与者另行安装 Node.js 或配置系统 `PATH`；`leetcode.nodePath` 只保留为显式覆盖选项。
...
1. 主机和所有来宾都从 [v0.19.3 Release](https://github.com/ChaosLights/vscode-leetcode/releases/tag/v0.19.3) 下载并在各自桌面版 VS Code 本地安装同一个 `vscode-leetcode-live-share-0.19.3.vsix`，不要把它安装到 Codespace 容器。
...
- 每位参与者只需要桌面版 VS Code 和此定制扩展；默认运行方式不需要本机另装 Node.js。
