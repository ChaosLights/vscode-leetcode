...
3. Run `npm run lint`, `npm test`, and `npm run test:vscode`. The VS Code integration suite must pass the host/remoted-guest CodeLens, local-command bridge, deletion-event recreation, stale-cache propagation, no-overwrite/staging ownership, symlink, and read-only cases.
...
7. With Live Share 1.1.122, open the same generated problem on a host and guest. Confirm each window shows one native CodeLens set and that Test/Submit use different local accounts.
