...
3. Run `npm run lint`, `npm test`, and `npm run test:vscode`. The VS Code integration suite must pass local CodeLens, Remote/Live Share local inline actions, deletion-event recreation, stale-cache propagation, no-overwrite/staging ownership, symlink, and read-only cases.
