...
5. Package the LeetCode extension and the activation-fixed Excalidraw companion with the pinned tools and audited upstream hash:

   ```bash
   npm run build
   sha256sum vscode-leetcode-live-share-<version>.vsix
   sha256sum excalidraw-editor-pairing-3.9.301.vsix
   ```
...
8. Commit and push the source, tag the exact commit as `v<version>`, and create a GitHub Release containing both VSIX files.
9. Download both release assets again and verify their SHA256 values before updating any consuming repository.
